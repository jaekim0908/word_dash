...
-(id) init {
	...
	if ((self = [super init])) {
        ...
        CCMenuItem *twitterMenuItem = [CCMenuItemImage
                                        itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"Twitter.png"] 
                                        selectedSprite:[CCSprite spriteWithSpriteFrameName:@"Twitter.png"] 
                                        target:self
                                        selector:@selector(twitterLink)];
        twitterMenuItem.position = ccp(440,290);

        CCMenuItem *facebookMenuItem = [CCMenuItemImage
                                       itemFromNormalSprite:[CCSprite spriteWithSpriteFrameName:@"Facebook.png"] 
                                       selectedSprite:[CCSprite spriteWithSpriteFrameName:@"Facebook.png"] 
                                       target:self
                                       selector:@selector(facebookLink)];
        facebookMenuItem.position = ccp(440, 235);

        
        CCMenu *starMenu = [CCMenu menuWithItems:singlePlayerMenuItem, playAndPassMenuItem, playWithFriendsMenuItem, howToPlayMenuItem, statsMenuItem, twitterMenuItem, facebookMenuItem, nil];
