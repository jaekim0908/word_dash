...
+ (void)applicationDidEnterBackground
{
	...
	OpenFeint* instance = [OpenFeint sharedInstance];
	if([instance->mLocation isLocationUpdating] && instance && instance->mLocation)
	{
		[instance->mLocation stopUpdatingLocation];
		OFSafeRelease(instance->mLocation);
		instance->appNeedsGetLocationOnForeground = YES;
	}
}
...
+ (void)applicationWillEnterForeground
{
	...
	OpenFeint* instance = [OpenFeint sharedInstance];
	if (instance && ![OpenFeint isSuccessfullyBootstrapped] && OFReachability::Instance()->isGameServerReachable())
	{
		OFDelegate noopSuccess;
		OFDelegate noopFailure;
	...
	if(instance && instance->appNeedsGetLocationOnForeground)
	{
		[OpenFeint startLocationManagerIfAllowed];
		instance->appNeedsGetLocationOnForeground = NO;
	}
}
